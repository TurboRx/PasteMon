...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return (
    <html lang="en" className="dark">
      ...
      <body className="min-h-screen bg-dark-900 text-dark-100 bg-grid antialiased">
        <header className="sticky top-0 z-50 glass-strong border-b border-dark-700/50">
          <nav className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6 sm:py-4">
            <Link href="/" className="group flex items-center gap-2 sm:gap-3 flex-shrink-0">
              <div className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-xl bg-gradient-to-br from-accent-purple to-accent-pink shadow-lg shadow-accent-purple/20 overflow-hidden">
                <img src="/logo.jpg" alt="PasteMon Logo" className="h-full w-full object-cover" />
              </div>
            ...
            <div className="flex items-center gap-3 sm:gap-6">
              <Link href="/browse" className="text-sm font-medium text-dark-200 transition-colors hover:text-white">
                Browse
              </Link>
              <Link href="/new" className="btn-primary text-sm py-2 px-4 sm:py-auto sm:px-6">
                + New Paste
              </Link>
            </div>
          </nav>
